...
<head>
    ...
    <title>Edit Blog Category | IslamicPrayerTimes</title>
...
<body>
                ...
                <div class="row">
                    <div class="col-lg-8">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <!-- FIXED: Use blogCategory variable name -->
                                <form action="{{ route('admin.blog-categories.update', $blogCategory->id) }}" method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="mb-3">
                                        <label for="categoryName" class="form-label">Category Name</label>
                                        <input type="text" class="form-control" id="categoryName" name="name" value="{{ old('name', $blogCategory->name) }}" required>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label for="categorySlug" class="form-label">Slug</label>
                                        <input type="text" class="form-control" id="categorySlug" name="slug" value="{{ old('slug', $blogCategory->slug) }}">
                                        <div class="form-text">URL-friendly version of the name.</div>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label for="categoryDescription" class="form-label">Description</label>
                                        <textarea class="form-control" id="categoryDescription" name="description" rows="4">{{ old('description', $blogCategory->description) }}</textarea>
                                    </div>
                    ...
                    <div class="col-lg-4">
                        <div class="card">
                            ...
                            <div class="card-body">
                                <div class="mb-3">
                                    ...
                                    <span class="badge bg-primary float-end">{{ $blogCategory->blogs_count }}</span>
                                </div>
                                <div class="mb-3">
                                    <strong>Created:</strong>
                                    <span class="float-end">{{ $blogCategory->created_at->format('M d, Y') }}</span>
                                </div>
                                <div class="mb-3">
                                    <strong>Last Updated:</strong>
                                    <span class="float-end">{{ $blogCategory->updated_at->format('M d, Y') }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mt-4">
                            ...
                            <div class="card-body">
                                ...
                                <!-- FIXED: Use blogCategory variable name -->
                                <form action="{{ route('admin.blog-categories.destroy', $blogCategory->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger w-100" onclick="return confirm('Are you sure you want to delete this category? This action cannot be undone.')">
    ...
    <script>
        ...
        document.getElementById('categoryName').addEventListener('input', function() {
            ...
            if (!slugField.value || slugField.value === '{{ $blogCategory->slug }}') {
