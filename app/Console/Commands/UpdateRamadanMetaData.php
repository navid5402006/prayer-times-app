...
class UpdateRamadanMetaData extends Command
{
    protected $signature = 'ramadan:update-meta 
                            {--chunk=50 : Number of records to process at once}
                            {--force : Force update all records}
                            {--city= : Update specific city only}';

    protected $description = 'Update all Ramadan meta data (title, description, keywords, main description) with accurate daily timings from ALADHAN API';

    public function handle()
    {
        $this->info('Starting Ramadan ALL META DATA update using ALADHAN API...');
        $this->newLine();
        ...
        if ($specificCity) {
            ...
            // Agar force nahi hai to sirf 3 din pehle update hone wali records
        ...
        $this->info("Found {$totalRecords} records to update ALL meta data.");
        ...
        $query->chunk($chunkSize, function ($records) use ($controller, $bar, &$updatedCount, &$errorCount) {
            foreach ($records as $record) {
                try {
                    ...
                    // Get calculation method from record or use default
                    ...
                    // Generate ALL meta data (title, description, keywords, main description)
                    ...
                    $errorCount++;
                    Log::error("Failed to update meta data for ID {$record->id}: " . $e->getMessage());
        ...
        $this->info("ALL Meta Data update completed!");
        $this->table(
            ['Status', 'Count'],
            [
    ...
    /**
     * Get today's times from ALADHAN API (same as controller)
     */
    private function getTodayTimesFromAladhan($latitude, $longitude, $year)
    {
        try {
            ...
            Log::error("ALADHAN API failed in command: " . $e->getMessage());
